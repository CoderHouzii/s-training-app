...
import songming.straing.widget.commentwidget.ClickableSpanEx;
import songming.straing.widget.commentwidget.SpannableStringBuilderAllVer;
...
public class PraiseWidget extends TextView {
    ...
    private List<UserDetailInfo> datas;
    ...
    public void setDatas(List<UserDetailInfo> datas) {
        this.datas = datas;
    }
    ...
    private void createSpanStringBuilder(List<UserDetailInfo> datas) {
        ...
        if (spanStrBuilder == null) {
            ...
            for (int i = 0; i < datas.size(); i++) {
                PraiseClick praiseClick = new PraiseClick.Builder(getContext(), datas.get(i)).setTextSize(textSize)
                                                                                             ...
                                                                                             .build();
                try {
                    spanStrBuilder.append(datas.get(i).username, praiseClick, 0);
                }catch (NullPointerException e){
                    e.printStackTrace();
                    Log.e(TAG,"praiseUserDetailInfo是空的哦");
                }
